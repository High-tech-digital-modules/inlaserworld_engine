...
void DEATHMATCH_playerAmmoChanged(uint8_t aPlayerIndex,
		uint16_t aOldValue,
		uint16_t aNewValue,
...
void DEATHMATCH_newLeader(const Player* apPLayer) {
...
void DEATHMATCH_setup(ColorT aColorArena) {
  gColorArena = aColorArena;
...
void DEATHMATCH_gameStateChanged(uint8_t aState){
...
void DEATHMATCH_lightGotHit(const char * aAddress, uint8_t aCode, uint8_t aInfo) {  
