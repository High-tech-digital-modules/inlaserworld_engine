...
function PlayerScore({ index} ) {
  ...
  return <SPlayerDataScoreRow bigger={gameResultState !== GameResultsState.FINISHED}>
    ...
    <SPlayerDataScoreRowItem>{player.agentPoints}</SPlayerDataScoreRowItem>
    <SPlayerDataScoreRowItem>{player.killsAsAgent}</SPlayerDataScoreRowItem>
    <SPlayerDataScoreRowItem>{player.becomeAgent}</SPlayerDataScoreRowItem>
  </SPlayerDataScoreRow>;
}
...
function Player ( {index} ) {
  ...
  return <SPlayerItem
    ...
    <SColorCircle style={{ backgroundColor:  player.isAgent ? customData.agentColor : customData.normalColor }} />
    <SRank>{(index + 1) + '. '}</SRank>
    ...
    <SContent>
      ...
      <div>
        <SPlayerDataHeader style={{ marginLeft: gameResultState === GameResultsState.RUNNING ? 260 : undefined }}>
          ...
          <SPlayerDataHeaderItem>
            <SPlayerTableHeaderItem>
              <SPlayerTableHeaderItemName style={{  fontSize: 14, width: 110 }}>
                Agent points
              </SPlayerTableHeaderItemName>
            </SPlayerTableHeaderItem>
          </SPlayerDataHeaderItem>
          <SPlayerDataHeaderItem>
            <SPlayerTableHeaderItem>
          ...
          <SPlayerDataHeaderItem>
            <SPlayerTableHeaderItem>
              <SPlayerTableHeaderItemName style={{  fontSize: 14, width: 100 }}>
                Become agent
              </SPlayerTableHeaderItemName>
            </SPlayerTableHeaderItem>
          </SPlayerDataHeaderItem>
    ...
    <SContent>
      {gameResultState !== GameResultsState.RUNNING && <SPlayerMain>
        ...
        <SExtremes bigger={gameResultState !== GameResultsState.FINISHED}>
          ...
          <h4>Most points being agent</h4>
          <div>{findExtremeByProp('agentPoints', gameResultState).playerName}</div>
          <h4>Most hits being agent</h4>
          <div>{findExtremeByProp('killsAsAgent', gameResultState).playerName}</div>
          <h4>Most affected agents</h4>
          <div>{findExtremeByProp('killsAsAgent', gameResultState).playerName}</div>
        </SExtremes>
      </SPlayerMain>}
      {teamNumber > 0 && <div>
